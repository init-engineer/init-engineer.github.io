...
!function(c){c.fn.simpleJekyllSearch=function(e){var n=c.extend({jsonFile:"/search.json",jsonFormat:"title,tags,categories,url,date",template:'<li><article><a href="{url}"><span class="entry-category">{categories}</span> {title} <span class="entry-date"><time datetime="{date}">{date}</time></span></a></article></li>',searchResults:".search-results",limit:"10",noResults:"<p>真糟糕，我們什麼都沒找到 :(</p>"},e),a=n.jsonFormat.split(","),o=[],l=this,i=c(n.searchResults);function r(){i.children().remove()}n.jsonFile.length&&i.length&&c.ajax({type:"GET",url:n.jsonFile,dataType:"json",success:function(e,t,s){o=e,l.keyup(function(e){var t,s,l;c(this).val().length?(s=c(this).val(),l=[],c.each(o,function(e,t){for(e=0;e<a.length;e++)void 0!==t[a[e]]&&-1!==t[a[e]].toLowerCase().indexOf(s.toLowerCase())&&(l.push(t),e=a.length)}),t=l,r(),i.append(c(n.searchResultsTitle)),t.length?c.each(t,function(e,t){if(e<n.limit){for(var s=n.template,e=0;e<a.length;e++){var l=new RegExp("{"+a[e]+"}","g");s=s.replace(l,t[a[e]])}i.append(c(s))}}):i.append(n.noResults)):r()})},error:function(e,t,s){console.log("***ERROR in simpleJekyllSearch.js***"),console.log(e),console.log(t),console.log(s)}})}}(Zepto);
